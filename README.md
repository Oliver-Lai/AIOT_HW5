...
- 🤖 **AI Detection**: Uses the "openai-community/roberta-base-openai-detector" model for accurate detection
- 📊 **Visual Results**: Clear progress bars and color-coded status messages
- 💯 **Probability Metrics**: Shows exact percentages for AI vs Human probability
- ⚡ **Fast & Efficient**: Optimized with model caching for quick responses
- 🎯 **Confidence Levels**: Displays High/Medium/Low confidence indicators
...
Streamlit Cloud 會在首次部署時自動下載並緩存模型。
...
3. **首次部署**
   - 首次部署需要 5-10 分鐘（下載模型）
   - Streamlit Cloud 會自動緩存模型
   - 後續重啟或更新會使用緩存（<1 分鐘）

4. **重要說明**
   - ✅ 模型會被 Streamlit Cloud 自動緩存
   - ✅ 不需要將模型文件提交到 Git
   - ✅ 更新代碼不會重新下載模型
   - ✅ 只有刪除應用重新部署才需要重新下載
...
2. **將 model_cache 文件夾壓縮（可選）**
   ```bash
   # 注意：這會創建一個 ~500MB 的壓縮文件
...
- Git 倉庫會變得很大（~500MB+）
- Clone 和 Pull 會很慢
- Streamlit Cloud 會自動處理模型緩存
- 免費的 Git LFS 配額有限

### 推薦做法

**✅ 最佳實踐**：讓 Streamlit Cloud 自動處理
- 第一次部署等待 5-10 分鐘
- 模型會被 Streamlit Cloud 永久緩存
- 代碼更新不會影響模型緩存
- 完全自動化，無需手動管理
...
- **Training**: Trained by OpenAI to detect GPT-2 generated text
...
1. **Model Specificity**: Trained specifically for GPT-2 detection by OpenAI; may not accurately detect newer AI models (GPT-3, GPT-4, Claude, etc.)
...
├── download_model.py       # Script to pre-download model
├── requirements.txt        # Python dependencies
├── README.md              # This file
├── AGENTS.md              # OpenSpec instructions
├── model_cache/           # Cached model files (auto-generated)
